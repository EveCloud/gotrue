...
WORKDIR /go/src/github.com/evecloud/auth
...
# Building stuff
COPY . /go/src/github.com/evecloud/auth
...
COPY --from=build /go/src/github.com/evecloud/auth/auth /usr/local/bin/auth
COPY --from=build /go/src/github.com/evecloud/auth/migrations /usr/local/etc/auth/migrations/
